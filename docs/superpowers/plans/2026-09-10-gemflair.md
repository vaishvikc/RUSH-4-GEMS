...
**Tech Stack:** Python 3.10+, polars, numpy, xgboost, torch + transformers (inference only), typer, pyyaml, pytest. External CLIs: `flair` (flair-benchmark), `cocoa` (cocoa-tokenizer), `cotorra`, plus `clifpy` as a library. Managed with `uv`.
...
**Note for the implementer:** `flair_benchmark.tasks.get(task).META` is the assumed accessor. Confirm it against `FLAIR/flair_benchmark/tasks/__init__.py` and adjust the one line if the real accessor differs.
